...
dat.model.r.DSF <- dat.model.r.DSF.tmp %>%
  select(recruit, siteid, source) %>%
  
  
  group_by(siteid, source) %>%
  
  summarize(ndead = sum(recruit == "dead")) %>%
...
dat.model.r.DSF <- dat.model.r.DSF %>%
    ...
    group_by(siteid) %>% 
    transmute(ndead=sum(ndead)) %>%
  
  distinct(siteid, ndead) %>%
...
dat.model.r.DSF <- dat.model.r.DSF %>%
 left_join(dat.sitelist %>% distinct(siteid, fireclass, droughtclass), by = "siteid") %>%
  ...
  tidyr::complete(dat.sitelist, fill = list(ndead = 0)) %>%
...
model1.code <- "model {
  for (i in 1:length(ndead)) {
    ndead[i] ~ dnegbin(p[treatment[i]], sz[treatment[i]])
    
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(ndead[i], p[treatment[i]], sz[treatment[i]])
  }
...
dat.model.DSF <- DAT.DSF %>%
  ...
  select(ndead, treatment = treat.index)
...
dat.obs <- DAT.DSF %>%
  left_join(TreatLookup, by = "droughtfire") %>%
  select(treat.factor, ndead)


ggplot() +
  geom_jitter(dat = dat.obs,
              aes(x = treat.factor, y = ndead),
  ...
  labs(x = "", y = "Number of dead juveniles",
       title = "DSF sites: Median expected number of dead juveniles",
...
model2.code <- "model {
  for (i in 1:length(ndead)) {
    ndead[i] ~ dnegbin(p[i], sz[treatment[i]])
    ...
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(ndead[i], p[i], sz[treatment[i]])
  }
...
## for use in replace_diff markdown
#effect1.DSF <- postndead3
...
DSF.ndead <- ggplot(data = dat.gg1, aes (x = var)) +
  
 My_Theme +
  
     geom_jitter(data = dat.model.r.DSF.plot, aes(y = ndead, col = var), 
            height = 0.2,
...
ggplot() +
  ...
  labs(x = "", y = "Number of dead juveniles",
       title = "WSF sites: Median expected number of dead juveniles",
       subtitle = paste("Intervals are 50% and 95% bounds.")) +
...
WSF.ndead <- ggplot(data = dat.gg1.b, aes (x = var)) +
  
 My_Theme +
  
     geom_jitter(data = dat.model.r.WSF.plot, aes(y = ndead, col = var), 
            height = 0.2,
            size = 0.8, alpha = 0.3, width = 0.25, show.legend = FALSE) +
  ...
  geom_linerange(data = x.stats1.b, aes(ymin = lwr50+0.1, ymax = upr50-0.1, color = var),
               size = 4, show.legend = FALSE) +
...
x <- dat.gg1.b %>%
  ...
  summarize(lwr95 = hpdi.vec(diff1.b, 0.95)[1],
            lwr50 = hpdi.vec(diff1.b, 0.5)[1],
            mid = median(diff1.b),
            upr50 = hpdi.vec(diff1.b, 0.5)[2],
            upr95 = hpdi.vec(diff1.b, 0.95)[2])
