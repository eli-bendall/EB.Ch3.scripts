...
dat.model.r.DSF <- dat.model.r.DSF.tmp %>%
  select(dbhclass, recruit, siteid) %>%
...
dat.model.r.DSF <- dat.model.r.DSF %>%
 left_join(dat.sitelist %>% distinct(siteid, fireclass, droughtclass), by = "siteid") %>%
  ...
  tidyr::complete(dat.sitelist, fill = list(nseed = 0, nsprout = 0)) %>%
...
## replace an NA with zero
```{r}
dat.model.r.DSF[is.na(dat.model.r.DSF)] <- 0

#multiply by 4 to estimate no. per site

dat.model.r.DSF$nseed <- dat.model.r.DSF$nseed * 4
dat.model.r.DSF$nsprout <- dat.model.r.DSF$nsprout * 4
dat.model.r.DSF$total <- dat.model.r.DSF$total * 4
...
model1.code <- "model {
  for (i in 1:length(nsprout)) {
    nsprout[i] ~ dnegbin(p[treatment[i]], sz[treatment[i]])
    
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(nsprout[i], p[treatment[i]], sz[treatment[i]])
  }
...
dat.model.DSF <- DAT.DSF %>%
  ...
  select(nsprout, treatment = treat.index)
...
dat.obs <- DAT.DSF %>%
  left_join(TreatLookup, by = "droughtfire") %>%
  select(treat.factor, nsprout)


ggplot() +
  geom_jitter(dat = dat.obs,
              aes(x = treat.factor, y = nsprout),
  ...
  labs(x = "", y = "Number of resprouts",
       title = "DSF sites: Median expected number of resprouts",
...
model2.code <- "model {
  for (i in 1:length(nsprout)) {
    nsprout[i] ~ dnegbin(p[i], sz[treatment[i]])
    ...
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(nsprout[i], p[i], sz[treatment[i]])
  }
...
Examining differences is a bit more complicated than the first example where we just had a single distribution of differences (between 640ppm and 400ppm CO2). One option is the standard regression approach of treating one factor level as the reference level and examining differences between this and the other levels. Here we set well-watered / ambient CO2 as the reference level:
...
My_Theme = theme(
  ...
  axis.text.x = element_text(size = 10.5, vjust = 0.9, hjust = 0.9, margin = unit(c(0.15, 0.1, 0.1, 0.1), "cm"), colour = rhg_cols2, angle = 45),
...
DSF.nsprout <- ggplot(data = dat.gg1, aes (x = var)) +
  
 My_Theme +
  
    geom_jitter(data = dat.model.r.DSF.plot, aes(y = nsprout), 
            height = 0.2,
            size = 2, alpha = 0.4, colour = "grey70") +
  ...
  geom_linerange(data = x.stats1, aes(ymin = lwr50, ymax = upr50),
               size = 5, show.legend = TRUE, position = position_dodge(width = 1), color = rhg_cols2) +
 
  labs(y = "", x = "", title = "Surviving resprouts") +
  
    scale_y_continuous(breaks = c(50, 100, 150, 200)) +
  coord_cartesian(ylim=c(50,200)) 

   ggsave("DSF.nsprout.png")
...
## replace an NA with zero
```{r}
dat.model.r.WSF[is.na(dat.model.r.WSF)] <- 0

#multiply by 4 to estimate no. per site

dat.model.r.WSF$nseed <- dat.model.r.WSF$nseed * 4
dat.model.r.WSF$nsprout <- dat.model.r.WSF$nsprout * 4
dat.model.r.WSF$total <- dat.model.r.WSF$total * 4
...
model1.loo.WSF <- loo::loo(post.WSF[, ii])
print(model1.loo.WSF)
...
ggplot() +
  ...
  labs(x = "", y = "Number of resprouts",
       title = "WSF sites: Median expected number of resprouts",
       subtitle = paste("Intervals are 50% and 95% bounds.")) +
...
model2.loo.WSF <- loo::loo(post2.WSF[, ii])
print(model2.loo.WSF)
...
WSF.nsprout <- ggplot(data = dat.gg1.b, aes (x = var)) +
  
 My_Theme +
  
    geom_jitter(data = dat.model.r.WSF.plot, aes(y = nsprout), 
            height = 0.2,
            size = 2, alpha = 0.4, colour = "grey70") +
