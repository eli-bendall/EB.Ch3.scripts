...
```{r}
dat.seed <- dat.model.r.DSF %>%
  group_by(droughtfire) %>%
  summarize(nseed = mean(nseed))
```
...
ggplot(data = dat.gg) +
  geom_jitter(aes(x = treat.factor, y = nseed),
              width = 0.1, height = 0, size = 3) +
  

  labs(x = "", y = "Seedlings",
       title = "New seedlings in DSF sites") +
...
model1.code <- "model {
  for (i in 1:length(nseed)) {
    nseed[i] ~ dnegbin(p[treatment[i]], sz[treatment[i]])
    
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(nseed[i], p[treatment[i]], sz[treatment[i]])
  }
...
dat.model.DSF <- DAT.DSF %>%
  left_join(TreatLookup, by = "droughtfire") %>%
  select(nseed, treatment = treat.index)
...
dat.obs <- DAT.DSF %>%
  left_join(TreatLookup, by = "droughtfire") %>%
  select(treat.factor, nseed)


ggplot() +
  geom_jitter(dat = dat.obs,
              aes(x = treat.factor, y = nseed),
  ...
  labs(x = "", y = "Number of seedlings",
       title = "DSF sites: Median expected number of seedlings",
...
model2.code <- "model {
  for (i in 1:length(nseed)) {
    nseed[i] ~ dnegbin(p[i], sz[treatment[i]])
    ...
    # monitor log-likelihood for WAIC calculation
    loglik[i] <- logdensity.negbin(nseed[i], p[i], sz[treatment[i]])
  }
...
DSF.nseed <- ggplot(data = dat.gg1, aes (x = var)) +
  
 My_Theme +
  
     geom_jitter(data = dat.model.r.DSF.plot, aes(y = nseed, col = var), 
            height = 0.2,
  ...
  geom_linerange(data = x.stats1, aes(ymin = lwr50+0.7, ymax = upr50-0.7, color = var),
 ...
 scale_color_manual(values = rhg_cols2) +
  ...
  labs(y = "", x = "", title = "Post-fire seedlings") +
...
ggplot() +
  ...
  labs(x = "", y = "Number of seedlings",
       title = "WSF sites: Median expected number of seedlings",
       subtitle = paste("Intervals are 50% and 95% bounds.")) +
...
My_Theme = theme(
  ...
  legend.key.width = unit(0.5, "cm"),
  legend.background = element_blank(),
  plot.margin = unit(c(2, 4, 6.5, 4), "pt"))


WSF.nseed <- ggplot(data = dat.gg1.b, aes (x = var)) +
  
 My_Theme +
  
     geom_jitter(data = dat.model.r.WSF.plot, aes(y = nseed, col = var), 
            height = 0.2,
            size = 0.8, alpha = 0.3, width = 0.25, show.legend = FALSE) +
  ...
  geom_linerange(data = x.stats1.b, aes(ymin = lwr50+0.4, ymax = upr50-0.4, color = var),
               size = 4, show.legend = FALSE) +
